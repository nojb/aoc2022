...
function doturn(t, i)
   secondhalf(t, firsthalf(t, i))
end

-- Return bounding rectangle containing all occupied cells.
function bounds(t)
   local xmin, xmax, ymin, ymax
   local function f(x, y)
      if not xmin or x < xmin then xmin = x end
      if not xmax or x > xmax then xmax = x end
...
-- Number of empty cells contained in the bounding rectangle.
function score(t)
   local xmin, xmax, ymin, ymax = bounds(t)
   return (xmax - xmin + 1) * (ymax - ymin + 1) - length(t)
end

function main()
   local t = read()
   for i = 1, 10 do doturn(t, i) end
   print(score(t))
end
